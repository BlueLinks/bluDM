...
export function DiceFormulaInput({
  ...
  function setNumber(field: "diceCount" | "fixedValue", delta: number) {
    ...
    onChange({
      ...value,
      [field]: String(Math.max(min, Number.isFinite(current) ? current + delta : delta)),
    });
  }
  return (
    <div className="grid gap-2 sm:grid-cols-[minmax(110px,1fr)_110px_minmax(110px,1fr)]">
      <StepperNumber
        ...
        onStep={(delta) => setNumber("diceCount", delta)}
      />
      <FieldlessSelect
        label="Die"
        value={value.dieSize}
        onChange={(dieSize) => onChange({ ...value, dieSize })}
      />
      <StepperNumber
        label="Modifier"
        value={value.fixedValue}
        min={-99}
        onChange={(fixedValue) => onChange({ ...value, fixedValue })}
        onStep={(delta) => setNumber("fixedValue", delta)}
      />
    </div>
  );
}
...
export function formatDiceFormula(value: {
  diceCount: string;
  dieSize: string;
  fixedValue: string;
}) {
...
function FieldlessSelect({
  ...
  return (
    <label className="grid gap-1 text-xs font-semibold uppercase text-muted-foreground">
      {label}
      <Select
        options={diceSizes.map((die) => ({ value: String(die), label: `d${die}` }))}
        placeholder="Die"
        value={value}
    ...
    <div className="rounded-lg border border-border bg-background p-3">
      ...
      <label className="flex items-center justify-between gap-3 rounded-md bg-muted/60 px-2 py-2 text-xs text-muted-foreground">
        ...
        <input
          ...
          type="checkbox"
          onChange={(event) => onSaveProficiencyChange?.(event.target.checked)}
        />
      </label>
    </div>
  );
  ...
  return (
    <div className="grid gap-3 xl:grid-cols-2 max-[1133px]:grid-cols-1">
      {groups.map((group) => (
        ...
        >
          <div className="grid grid-cols-[4.25rem_4rem_3.5rem_5.25rem] border-b border-border bg-muted/60 text-center text-[0.62rem] font-black uppercase leading-none text-muted-foreground">
            ...
            <span className="px-2 py-2">Save Prof</span>
          </div>
          {group.map((ability) => {
            ...
            return (
              <div
                className="grid grid-cols-[4.25rem_4rem_3.5rem_5.25rem] items-stretch border-b border-border last:border-b-0"
                key={ability.key}
              >
                <span className="grid items-center border-r border-border bg-muted/30 px-2 font-black uppercase text-muted-foreground">
                  {ability.label}
                </span>
                <span className="grid place-items-center border-r border-border bg-primary/5 px-1">
                  <Input
                    className="h-9 w-14 rounded-md text-center font-semibold"
                    type="number"
...
export function SkillsTable({
  ...
  return (
    <div className="grid gap-2 xl:grid-cols-2 max-[1133px]:grid-cols-1">
      {skillDefinitions.map((skill) => {
        ...
        return (
          <div
            className="grid min-w-0 grid-cols-[1.8rem_2.1rem_minmax(4.5rem,1fr)_2.3rem_2.7rem_3rem] items-center gap-1.5 rounded-md border border-border bg-background px-2 py-1.5 text-sm"
            key={skill.name}
          >
            ...
            <label className="flex min-w-0 items-center justify-between gap-1 rounded bg-muted/60 px-1 py-1 text-[0.58rem] font-semibold uppercase text-muted-foreground">
              Prof
              <input
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-3">
      <DamageChecklist
        ...
        hint="Double damage is taken"
        marker="V"
        selected={damageVulnerabilities}
        field="damageVulnerabilities"
        onChange={onChange}
      />
      <DamageChecklist
        title="Damage Resistances"
        hint="Half of the damage is taken"
        marker="R"
      ...
      <div className="grid gap-2">
        {damageTypes.map((damage) => (
          ...
          >
            ...
            <input
              className="h-4 w-4 accent-primary"
              checked={selected.includes(damage.id)}
              type="checkbox"
              onChange={(event) => onChange(field, damage.id, event.target.checked)}
            />
          </label>
        ))}
      </div>
...
export function AbilitySelect({
  ...
  return (
    <Select
      options={abilities.map((ability) => ({ label: ability.label, value: ability.key }))}
      placeholder="None"
      value={value}
      onValueChange={onChange}
    />
