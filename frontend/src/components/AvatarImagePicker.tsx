...
export function AvatarImagePicker({ assetId, label, name, onChange, uploadImage, url }: AvatarImagePickerProps) {
  ...
  const dragRef = useRef<{ pointerId: number; x: number; y: number; pan: { x: number; y: number } } | null>(null);
  ...
  const proxiedURLSrc = activeURL && !urlValidationError ? `/api/assets/image-proxy?url=${encodeURIComponent(activeURL)}` : "";
  const sourceSrc = filePreviewURL || proxiedURLSrc || src;
  const sourceKind = filePreviewURL ? "file" : activeURL && !urlValidationError ? "url" : src ? "existing" : "none";
  const cropActive = sourceKind === "file" || cropEdited;
  const canSave = Boolean(sourceSrc && sourceKind !== "none" && !urlValidationError && (filePreviewURL || activeURL || cropEdited));
  ...
  const baseScale = orientedWidth && orientedHeight ? Math.max(avatarCropSize / orientedWidth, avatarCropSize / orientedHeight) : 1;
  ...
  async function saveAvatar() {
    ...
    try {
      if (cropActive) {
        ...
        const blob = await cropAvatarImage(image, naturalSize, zoom, clampedPan, rotation, flipX, flipY);
  ...
  function handleCropPointerDown(event: ReactPointerEvent<HTMLDivElement>) {
    if (!sourceSrc) return;
    beginCropEdit();
    dragRef.current = { pointerId: event.pointerId, x: event.clientX, y: event.clientY, pan: clampedPan };
  ...
  return (
    <AvatarPickerShell
      ...
      name={name}
      src={src}
      clearAction={src ? <Button type="button" size="sm" variant="ghost" onClick={handleClear}>Clear</Button> : null}
      changeAction={(
        <Modal open={open} onOpenChange={(nextOpen) => { setOpen(nextOpen); if (nextOpen) resetDraft(); }} title={`Set ${label.toLowerCase()}`} trigger={<Button type="button" size="sm" variant="secondary">Change avatar</Button>}>
          <AvatarDialogBody
            canSave={canSave}
            ...
            uploading={uploading}
            onCancel={() => { resetDraft(); setOpen(false); }}
            onClear={handleClear}
            onFile={prepareUpload}
            onFlipX={() => beginCropEdit() && setFlipX((current) => !current)}
            onFlipY={() => beginCropEdit() && setFlipY((current) => !current)}
            onImageError={() => setPreviewError("That image could not be previewed. Check the URL or choose a different image.")}
            onImageLoad={(image) => { setPreviewError(""); setNaturalSize({ width: image.naturalWidth, height: image.naturalHeight }); }}
    ...
    <section className="grid gap-3 rounded-lg border border-border bg-card p-3">
      <header className="flex items-center gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-sm text-muted-foreground">Upload a cropped square, use an image URL, or clear the image.</p>
          <div className="mt-2 flex flex-wrap gap-2">{changeAction}{clearAction}</div>
        </div>
      </header>
    </section>
  );
...
function AvatarThumb({ name, src }: { name: string; src: string }) {
  return (
    <figure className="grid h-20 w-20 shrink-0 place-items-center overflow-hidden rounded-lg border border-border bg-muted text-lg font-bold text-muted-foreground">
      {src ? <img className="h-full w-full object-cover" src={src} alt="" /> : name.slice(0, 2).toUpperCase() || "AV"}
    </figure>
  );
}

function clampAvatarPan(pan: { x: number; y: number }, displayWidth: number, displayHeight: number) {
...
function cropAvatarImage(
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      reject(new Error("This image host blocks cropping. Use the URL directly, or upload the image file instead."));
    }
  });
}
