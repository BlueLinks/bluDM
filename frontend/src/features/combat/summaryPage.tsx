...
export function EncounterSummaryPage() {
  ...
  useEffect(() => {
    ...
    void api.encounterRun(runID).then((payload) => {
      ...
      const players = (payload.run.combatants ?? []).filter(
        (combatant) => combatant.sourceType === "player",
      );
      const totalXP = defeatedEnemyXP(payload.run.combatants ?? []);
      const split = players.length ? Math.floor(totalXP / players.length) : 0;
      setXpAwards(
        Object.fromEntries(players.map((player) => [player.playerId || player.id, split])),
      );
    });
  }, [runID]);
    ...
    <Page>
      ...
      <Breadcrumbs
        items={[
          ...
          ...(encounter
            ? [{ label: encounter.name, to: `/campaigns/${encounter.campaignId}` }]
            : [{ label: "Encounter Run", to: `/encounter-runs/${run.id}` }]),
          { label: "Summary" },
        ]}
      />
      <PageHeader
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <SectionPanel title="XP Awards" icon={Sparkles}>
          <p className="mb-3 text-sm text-muted-foreground">
            Defeated enemy XP: {defeatedEnemyXP(run.combatants ?? [])}
          </p>
          <div className="grid gap-2">
            {players.map((player) => (
              <Field key={player.id} label={player.displayName}>
                <Input
                  type="number"
                  value={xpAwards[player.playerId || player.id] ?? 0}
                  onChange={(event) =>
                    setXpAwards({
                      ...xpAwards,
                      [player.playerId || player.id]: Number(event.target.value) || 0,
                    })
                  }
                />
              </Field>
        ...
        <SectionPanel title="Loot Pool" icon={Package}>
          <div className="mb-3 grid grid-cols-[1fr_auto] gap-2">
            <Input
              placeholder="Loot note or item"
              value={lootText}
              onChange={(event) => setLootText(event.target.value)}
            />
            <Button icon={Plus} onClick={addLoot}>
              Add
            </Button>
          </div>
          <div className="grid gap-2">
          ...
          {players.map((player) => (
            ...
            >
              ...
              <div className="mt-2 grid gap-1 text-sm text-muted-foreground">
                {(lootAssignments[player.playerId || player.id] ?? []).map((item) => (
